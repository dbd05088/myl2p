...
def l2_normalize(x, axis=None, epsilon=1e-12):
    ...
    x_inv_norm = np.reciprocal(np.sqrt(np.maximum(square_sum, epsilon)))
    return x * x_inv_norm
...
def expand_to_batch(x, batch_size: int, axis=0):
    ...
    return np.tile(np.expand_dims(x, axis=axis), shape)
...
class Prompt(nn.Module):
    ...
    def __call__(self, x_embed, prompt_mask=None, task_id=-1, cls_features=None, label=None):
        print("prompt_call")
        ...
        if self.prompt_pool:
            if self.use_prefix_tune_for_e_prompt:  # use prefix style
                ...
                if self.same_key_value:
                    ...
                    if self.prompt_init == "normal":
                        prompt = torch.nn.init.normal_(w, mean=0.0, std=0.01)     
                    else:
                        prompt = nn.init.uniform_(w, 0, 0.01)
                        print("not normal", self.prompt_init)
                    prompt = np.tile(prompt, (1, 2, 1, 1, 1, 1)) # why? in prefix tuning??
                else:
                ...
                # # of prompt = pool_size
                w = torch.empty(self.num_layers, self.pool_size, self.length, embed_dim)
                if self.prompt_init == "normal":
                    prompt = torch.nn.init.normal_(w, mean=0.0, std=0.01)     
                else:
                    prompt = nn.init.uniform_(w, 0, 0.01)
            ...
            if self.embedding_key == "mean":
                x_embed_mean = np.mean(x_embed, axis=1)  # bs, emb
            elif self.embedding_key == "max":
                x_embed_mean = np.max(x_embed, axis=1)
            elif self.embedding_key == "mean_max":
                x_embed_mean = np.max(x_embed, axis=1) + 2 * np.mean(x_embed, axis=1)
            ...
            if self.prompt_key:
                ...
                if self.prompt_key_init == "zero":
                    ...
                    prompt_key = nn.init.uniform_(w, 0, 0.01)
            else:
                # only compatible with prompt, not prefix
                prompt_mean = np.mean(prompt, axis=[0, 2])  # pool_size, emb
            ...
            sim = np.matmul(prompt_key_norm, np.transpose(
                x_embed_norm))  # pool_size, bs or pool_size, #class, bs
            sim = np.transpose(sim)  # bs, pool_size
            ...
            batched_key_norm = np.take(
                prompt_key_norm, idx, axis=0)  # bs, top_k, embed_dim / From prompt_key_norm, get data of idx.
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                batched_prompt_raw = np.take(prompt, idx, axis=2)  # num_layers, bs, allowed_size, prompt_len, embed_dim
                num_layers, bs, dual, allowed_size, prompt_len, num_heads, heads_embed_dim = batched_prompt_raw.shape
                batched_prompt = np.reshape(batched_prompt_raw,
                                            (num_layers, bs, dual, allowed_size *
                                            prompt_len, num_heads, heads_embed_dim))
            else:
                batched_prompt_raw = np.take(
                    prompt, idx,
            ...
            reduce_sim = np.sum(sim_pull) / x_embed.shape[0] # divide by bs
            ...
            if self.use_prefix_tune_for_e_prompt:  # use prefix style
                ...
                if self.same_key_value:
                    prompt = self.param("prompt", self.prompt_init,
                                        ...
                                        embed_dim // self.num_heads)) # prompt init's dimension
                    prompt = np.tile(prompt, (1, 2, 1, 1, 1)) # prompt => (1, 2, 1, 1, 1) dimension array
                else:
                    prompt = self.param("prompt", self.prompt_init,
