...
class l2p:
    ...
    def load_pretrain_state_dict(self):
        ...
        # 1.5 pre_trained_dict에 나머지 param 추가
    ...
    def model_forward(self, x, y):
        ...
        if do_cutmix:
            ...
            if self.use_amp:
                ...
                #print("return origianl")
                #print(self.model.state_dict()['prompt_pool_module.prompt'][0][0][0][:5])
                logit = res['logits']
            ...
            if self.use_amp:
                ...
                #print("origianl")
                #print(self.model.state_dict()['prompt_pool_module.prompt'][0][0][0][:7])
                self.model.load_state_dict(pretrained_model_dict)
                self.model.eval()
                res_pre = self.model(x)
                cls_features = res_pre['pre_logits']
